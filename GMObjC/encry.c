 ...
 * AES加密ECB模式 128
 */
int encrypt_AES_128_ECB(unsigned char *plaintext, int plaintext_len, const char *key, unsigned char *ciphertext) //, int olen)
{
    if (strlen(key) == 0) {
        return 0;
    }
     ...
     * IV size for *most* modes is the same as the block size. For AES this
     * is 128 bits */
    if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, aes_keybuf, iv))
        handleErrors();
 ...
 * AES解密ECB模式 128
 */
int decrypt_AES_128_ECB(unsigned char *ciphertext, int ciphertext_len, const char *key, unsigned char *plaintext)
{
    if (strlen(key) == 0) {
        return 0;
    }
     ...
     * IV size for *most* modes is the same as the block size. For AES this
     * is 128 bits */
    if (1 != EVP_DecryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, aes_keybuf, iv))
        handleErrors();
